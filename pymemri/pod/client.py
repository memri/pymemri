...
logging.basicConfig(format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                    ...
                    level=logging.INFO,
                    handlers=[
                        # logging.FileHandler('pymemri_{:%Y-%m-%d}.log'.format(datetime.now())),
                        logging.StreamHandler()
                    ])

# Cell

logger = logging.getLogger('PodClient')
...
class PodClient:
    ...
    def add_to_schema(self, *items: List[Union[object, type]]):
        ...
        try:
            ...
            logger.error(f"Exception in add to schema: {e}")
            return False
    ...
    def _load_photo_data(self, photo, size=None):
        if len(photo.file) > 0 and photo.data is None:
            ...
            if file is None:
                logger.error(f"Could not load data of {photo} attached file item does not have data in pod")
                return
    ...
    def gather_batch(items, start_idx, start_size=0, max_size=5000000):
        ...
        for i, x in enumerate(items):
            if i < idx:
                ...
                idx = i + 1
                logger.debug("Could not add item: Item exceeds max item size")
    ...
    def bulk_action(
        ...
        while not (
            ...
            try:
                ...
                logger.error(f"Could not complete bulk action, aborting: {e}")
                return False
    ...
    def create_edge(self, edge):
        ...
        try:
            ...
            logger.error(f"Exception in create edge {edge_dict['_name']}: {e}")
            return False
    ...
    def get_edges(self, id):
        try:
            ...
            logger.error(f"Exception in get edges {id}: {e}")
            return
    ...
    def _get_item_with_properties(self, id):
        try:
            ...
            logger.error(f"Exception in getting item {id}: {e}")
            return
    ...
    def update_item(self, node):
        ...
        try:
            ...
            logger.error(f"Exception in update item {node}: {e}")
            return False
    ...
    def exists(self, id):
        try:
            ...
            logger.error(f"Exception in exists {id}: {e}")
            return False
    ...
    def search_paginate(self, fields_data, limit=50, include_edges=True):
        ...
        try:
            ...
            logger.error(f"Exception in search paginate: {e}")
    ...
    def send_email(self, to, subject="", body=""):
        try:
            ...
            logger.error(f"Exception in send email: {e}")
            return False
