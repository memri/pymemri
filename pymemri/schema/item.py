...
class _ItemMeta(ModelMetaclass):
    ...
    generated by pyright/pylance signature.
    ...
    def __new__(mcs, name, bases, namespace, **kwargs) -> Any:
        ...
        res.__private_attributes__ = {**res.__private_attributes__, **mcs._itembase_private_attrs}
        res.__slots__ = res.__slots__ | mcs._itembase_private_attrs.keys()
        return res
...
class Edge(GenericModel, Generic[TargetType], smart_union=True):
    name: str
    source: Any
    target: TargetType
    ...
    def validate_target(cls, val: Any) -> TargetType:
        ...
        for ttype_str in cls.get_target_types_as_str():
            if val_type_str == ttype_str:
                return val
        ttype_display = cls.__fields__["target"]._type_display()
        raise ValueError(f"target with type `{val_type_str}` is not a `{ttype_display}`")
